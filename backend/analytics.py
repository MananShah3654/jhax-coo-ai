...
def _carts_supported(src) -> bool:
    ...
    """
    return callable(getattr(src, "carts", None))


def _capacity(src, branch_id: str | None = None) -> Dict[str, int]:
...
def carts_between(src, start: datetime, end: datetime, branch_id: str | None = None) -> List[Dict]:
    out = []
    for c in _carts(src):
        ts = _parse(c["ts"])
...
def kpi_window(src, start: datetime, end: datetime, branch_id: str | None = None) -> Dict:
    ...
    if _carts_supported(src):
        abandoned = len(carts_between(src, start, end, branch_id))
...
def _orders_upto(src, day_start: datetime, hour: int, branch_id: str | None = None) -> List[Dict]:
    """Orders on `day_start`'s day from 00:00 up to and INCLUDING `hour`."""
    return orders_between(src, day_start, day_start + timedelta(hours=hour + 1), branch_id)


def _returning_rate(src, window_orders: List[Dict], before: datetime) -> float:
...
def hourly_baseline(src, hour: int, lookback_days: int = 28, branch_id: str | None = None) -> float:
    ...
    for d in days:
        ...
        total += sum(o["total"] for o in orders_between(src, start, start + timedelta(hours=1), branch_id))
    return round(total / len(days), 2)


def detect_anomaly(src, checkpoint_hour: int, threshold: float = -0.15,
    ...
    today_cum = round(
        sum(o["total"] for o in orders_between(
            src, today, today + timedelta(hours=checkpoint_hour + 1), branch_id)),
        2,
    )
    baseline_cum = round(
        sum(hourly_baseline(src, h, branch_id=branch_id) for h in range(0, checkpoint_hour + 1)), 2)
...
def score_causes(src, checkpoint_hour: int | None = None,
    ...
    today_win = _orders_upto(src, today, checkpoint_hour, branch_id)
    base_wins = [_orders_upto(src, d, checkpoint_hour, branch_id) for d in base_days]
    ...
    r_today = _returning_rate(src, today_win, today)
    r_rates = [_returning_rate(src, w, d) for w, d in zip(base_wins, base_days)]
...
def revenue_diagnosis(src, checkpoint_hour: int | None = None,
                      branch_id: str | None = None) -> Dict:
    """The full "why did revenue drop" packet injected into restaurant_context."""
    if checkpoint_hour is None:
        checkpoint_hour = min(22, max(7, datetime.now(timezone.utc).hour))
    anomaly = detect_anomaly(src, checkpoint_hour, branch_id=branch_id)
    causes = score_causes(src, checkpoint_hour, branch_id=branch_id)
    ...
    return {
        ...
        "unavailable_signals": _unavailable_signals(src),
