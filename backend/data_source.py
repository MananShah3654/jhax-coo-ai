...
# ---------- Knowlwood (live menu adapter — used when DATA_SOURCE=knowlwood) ----------
# Cost is not provided by the menu API, so we estimate food cost by category
# to keep margin analytics meaningful. Beverages carry a lower food cost.
_FOOD_COST_PCT = 0.32
...
class KnowlwoodDataSource:
    """Live adapter for the Knowlwood menu API.

    Fetches the real menu catalog from KNOWLWOOD_API_URL and maps it into the
    internal menu shape (id, name, category, price, cost). Branches, customers
    and orders are then generated deterministically *against the real menu* so
    every downstream analytic (menu performance, KPIs, AI context) reflects the
    live catalog. Falls back to the built-in demo menu if the API is
    unreachable so the COO always boots.
    """
    ...
    def __init__(self) -> None:
        self.api_url = os.environ.get(
            "KNOWLWOOD_API_URL", "https://uat.knowlwoodusa.com/api"
        ).rstrip("/")
        self.restaurant = os.environ.get("KNOWLWOOD_RESTAURANT", "Knowlwood")
        self._fallback = MockDataSource()
        self._dataset: Dict | None = None
        self._categories: List[Dict] | None = None

    def _fetch_json(self, path: str) -> object | None:
        try:
            ...
            logger.warning("Knowlwood fetch failed %s%s: %s", self.api_url, path, exc)
            return None

    def _map_menu(self, raw_items: List[Dict]) -> List[Dict]:
        out: List[Dict] = []
        for it in raw_items:
            name = it.get("name") or ""
            if name.startswith("[DELETED]") or it.get("available") is False:
                continue
            price = round((it.get("price") or 0) / 100.0, 2)  # cents -> dollars
            category = (it.get("category") or {}).get("name") or "Uncategorized"
            out.append({
                "id": it.get("id"),
                "name": name,
                "category": category,
    ...
    def _build(self) -> Dict:
        if self._dataset is None:
            ...
            raw = self._fetch_json("/menu/items")
            if isinstance(raw, list) and raw:
                menu = self._map_menu(raw)
                logger.info("Knowlwood: loaded %d live menu items", len(menu))
            else:
                menu = self._fallback.menu()
                logger.warning("Knowlwood: using fallback menu")
            owner = {"name": os.environ.get("OWNER_NAME", "Manan"),
                     "restaurant": self.restaurant}
            self._dataset = build_dataset(menu=menu, owner=owner)
        return self._dataset

    def categories(self) -> List[Dict]:
        if self._categories is None:
            data = self._fetch_json("/menu")
            cats = data.get("categories") if isinstance(data, dict) else None
            self._categories = cats or []
        return self._categories

    def branches(self) -> List[Dict]:  return self._build()["branches"]
    def menu(self) -> List[Dict]:       return self._build()["menu"]
    def customers(self) -> List[Dict]:  return self._build()["customers"]
    def orders(self) -> List[Dict]:     return self._build()["orders"]
    def owner(self) -> Dict:            return self._build()["owner"]
...
# so every *_money amount is divided by 100. Square does not expose a food cost,
# so we reuse the same category-based estimate as the Knowlwood adapter to keep
# margin analytics meaningful. Bump _SQUARE_VERSION to the current value from the
# Square API changelog (https://developer.squareup.com/docs/build-basics/versioning)
# whenever you need newer fields.
